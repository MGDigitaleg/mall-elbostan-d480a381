...
export function MapFilters({
  ...
  return (
    <div dir="rtl" className="rounded-xl border border-border bg-card p-3" style={{ boxShadow: "0 1px 3px hsl(220 30% 10% / 0.04)" }}>
      <div className="grid gap-2.5 md:grid-cols-2 lg:grid-cols-[1fr_auto_auto_auto]">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="h-10 w-full rounded-xl border border-border bg-background pr-10 pl-3 text-[0.84rem] font-medium text-foreground placeholder:text-muted-foreground/50 outline-none transition-all focus:border-primary/40 focus:ring-2 focus:ring-primary/10 focus:shadow-sm text-right"
          />
        </div>

        {/* Category */}
        <Select value={categoryFilter} onValueChange={(v) => onCategoryChange(v as "all" | MallCategory)} dir="rtl">
        ...
        {/* Status */}
        <Select value={statusFilter} onValueChange={(v) => onStatusChange(v as "all" | MallUnitStatus)} dir="rtl">
          <SelectTrigger className="h-10 rounded-xl border-border bg-background text-[0.84rem] font-medium">

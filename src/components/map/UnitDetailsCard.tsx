...
const statusBadge: Record<MallUnitStatus, { bg: string; border: string; text: string }> = {
  occupied: { bg: "hsl(222 20% 94%)", border: "hsl(222 20% 84%)", text: "hsl(222 30% 36%)" },
  available: { bg: "hsl(24 90% 95%)", border: "hsl(24 85% 75%)", text: "hsl(24 85% 35%)" },
  coming_soon: { bg: "hsl(190 60% 93%)", border: "hsl(190 50% 72%)", text: "hsl(190 60% 30%)" },
};
...
function RewardBanner({ ctx }: { ctx: ActiveRewardContext }) {
  return (
    <div className="rounded-xl p-4" style={{ background: "hsl(222 40% 96%)", border: "1px solid hsl(222 30% 86%)" }}>
      <div className="flex items-center gap-2 text-[0.72rem] font-bold uppercase tracking-widest" style={{ color: "hsl(222 58% 38%)" }}>
        <Gift className="h-3.5 w-3.5" />
        مكافأة نشطة
      </div>
      <p className="mt-1.5 text-[0.88rem] font-bold text-foreground">{ctx.reward.title_ar}</p>
      {ctx.storeName && (
        <p className="mt-1 text-[0.8rem] text-muted-foreground">من {ctx.storeName}</p>
      )}
      {ctx.reward.claim_rules_ar && (
        <p className="mt-2 text-[0.78rem] leading-6 text-muted-foreground">{ctx.reward.claim_rules_ar}</p>
      )}
      {ctx.isCategory && (
        <p className="mt-2 text-[0.74rem] font-bold" style={{ color: "hsl(222 58% 38%)" }}>جميع المتاجر المُظلّلة مشاركة في هذا العرض</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[0.72rem] font-bold uppercase tracking-widest text-muted-foreground">رقم الوحدة</p>
          <p className="mt-1 text-xl font-extrabold text-foreground">{unit.code}</p>
        </div>
        <span
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {[
          ...
          <div key={item.label} className="rounded-lg p-3" style={{ background: "hsl(var(--secondary) / 0.5)", border: "1px solid hsl(var(--border))" }}>
            <div className="flex items-center gap-1.5 text-muted-foreground">
              <item.icon className="h-3 w-3" />
              <span className="text-[0.7rem] font-semibold">{item.label}</span>
            </div>
            <p className="mt-0.5 text-[0.85rem] font-bold text-foreground">{item.value}</p>
          </div>
        ))}
      </div>
      ...
      {/* Description */}
      {unit.description && (
        <div className="rounded-lg p-3" style={{ background: "hsl(var(--secondary) / 0.3)", border: "1px solid hsl(var(--border))" }}>
          <p className="text-[0.8rem] leading-6 text-muted-foreground">{unit.description}</p>
        </div>
      )}
        ...
        {unit.status === "available" ? (
          <>
            <Link to="/leasing" className="block">
              <Button variant="orange" className="h-11 w-full rounded-xl font-bold text-[0.88rem]">
          ...
          <>
            <Link to="/stores" className="block">
              <Button variant="default" className="h-11 w-full rounded-xl text-[0.88rem] font-bold">
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        {[
          ...
          <div key={item.label} className="flex items-center justify-between rounded-lg px-3.5 py-2.5 text-[0.8rem]" style={{ background: item.bg }}>
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 shadow-[var(--shadow-card)]">
      <div className="mb-4 flex items-center gap-2">
        <div className="h-[3px] w-5 rounded-full" style={{ background: unit ? "hsl(24 85% 50%)" : "hsl(222 30% 70%)" }} />
        <h2 className="text-[0.78rem] font-bold uppercase tracking-widest text-muted-foreground">

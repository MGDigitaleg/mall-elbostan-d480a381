...
import {
  ...
  Share2, Star, Shield, Wifi, Monitor, Gamepad2,
} from "lucide-react";
    ...
    <MainLayout>
      ...
      <section className="relative overflow-hidden" style={{ background: "var(--gradient-hero)" }}>
        ...
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute left-1/4 top-1/3 h-[500px] w-[500px] rounded-full opacity-[0.06]"
               style={{ background: `radial-gradient(circle, hsl(${storyColor}), transparent 70%)` }} />
          <div className="absolute bottom-0 right-1/4 h-[350px] w-[550px] rounded-full opacity-[0.04]"
        ...
        <div className="container relative max-w-6xl pb-16 pt-6 md:pb-24 md:pt-10">
          {/* Breadcrumb */}
          ...
          <div className="grid items-center gap-8 lg:grid-cols-[1fr_0.9fr] lg:gap-16">
            ...
            <motion.div variants={fadeUp} initial="hidden" animate="visible" className="space-y-6">
              ...
              <div className="flex items-center gap-5">
                <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl p-2.5"
                     style={{
                       background: "hsl(0 0% 100% / 0.07)",
                       border: "1px solid hsl(0 0% 100% / 0.12)",
                       boxShadow: "0 8px 32px hsl(220 60% 5% / 0.4), inset 0 1px 0 hsl(0 0% 100% / 0.06)",
                     }}>
                  {store.logo_url ? (
              ...
              <div className="flex flex-wrap items-center gap-2.5">
                {store.category && (
                  <span className="inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-[0.74rem] font-semibold"
                        style={{
                          background: `hsl(${storyColor} / 0.12)`,
                          border: `1px solid hsl(${storyColor} / 0.25)`,
                ...
                {store.unit_code && (
                  <span className="inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-[0.74rem] font-bold"
                        style={{ background: "hsl(var(--primary) / 0.12)", border: "1px solid hsl(var(--primary) / 0.25)", color: "hsl(var(--primary))" }}>
                    <Building2 className="h-3.5 w-3.5" />وحدة {store.unit_code}
                  </span>
                )}
                <span className={`inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-[0.74rem] font-semibold`}
                      style={{
                  ...
                  <div className={`h-2 w-2 rounded-full ${store.status === "available" ? "bg-orange-400" : "bg-emerald-400"}`}
                       style={{ boxShadow: `0 0 8px ${store.status === "available" ? "hsl(25 95% 55% / 0.5)" : "hsl(155 70% 40% / 0.5)"}` }} />
              ...
              {(store.short_description_ar || store.long_description_ar) && (
                <p className="max-w-lg text-[0.9rem] leading-[2] text-white/40">
              ...
              <div className="flex flex-wrap gap-3 pt-1">
                {store.whatsapp && (
                  <a href={`https://wa.me/${store.whatsapp}`} target="_blank" rel="noopener noreferrer">
                    <Button variant="cta" className="h-12 gap-2.5 rounded-xl px-7 text-[0.84rem] font-bold shadow-lg shadow-primary/25">
                      <MessageCircle className="h-[18px] w-[18px]" />تواصل واتساب
                    </Button>
                  </a>
                )}
                {store.phone && (
                  <a href={`tel:${store.phone}`}>
                    <Button className="h-12 gap-2.5 rounded-xl px-7 text-[0.84rem] font-bold text-white transition-all hover:bg-white/12"
                            style={{ background: "hsl(0 0% 100% / 0.08)", border: "1px solid hsl(0 0% 100% / 0.15)" }}>
                      <Phone className="h-[18px] w-[18px]" />اتصل الآن
                    </Button>
                  </a>
                )}
                <Link to="/map">
                  <Button className="h-12 gap-2.5 rounded-xl px-7 text-[0.84rem] font-bold text-white/60 transition-all hover:text-white hover:bg-white/8"
                          style={{ background: "hsl(0 0% 100% / 0.04)", border: "1px solid hsl(0 0% 100% / 0.1)" }}>
                    <Compass className="h-[18px] w-[18px]" />الخريطة
                  </Button>
                </Link>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              <div className="relative aspect-[4/3] overflow-hidden rounded-2xl"
                   style={{ boxShadow: "0 16px 64px hsl(220 60% 5% / 0.6), 0 4px 12px hsl(220 60% 5% / 0.3)" }}>
                <img src={heroImage} alt={store.name_ar} className="h-full w-full object-cover" loading="eager" />
                <div className="absolute inset-0 rounded-2xl ring-1 ring-inset ring-white/10" />
                <div className="absolute inset-0" style={{ background: "linear-gradient(to top, hsl(222 44% 6% / 0.6), transparent 50%)" }} />
                ...
                <button
                  ...
                  className="absolute left-3 top-3 flex h-10 w-10 items-center justify-center rounded-xl transition-all hover:scale-105"
                  style={{ background: "hsl(0 0% 0% / 0.45)", backdropFilter: "blur(16px)", border: "1px solid hsl(0 0% 100% / 0.15)" }}>
                  <Share2 className="h-4 w-4 text-white/80" />
                </button>

                {/* Bottom info overlay */}
                {store.unit_code && (
                  <div className="absolute bottom-3 right-3 flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[0.7rem] font-bold text-white/80"
                       style={{ background: "hsl(0 0% 0% / 0.45)", backdropFilter: "blur(16px)", border: "1px solid hsl(0 0% 100% / 0.12)" }}>
                    <MapPin className="h-3 w-3" />وحدة {store.unit_code}
                  </div>
                )}
              </div>
            </motion.div>
        ...
        <div className="container max-w-6xl py-10 md:py-16">
          <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:gap-12">
            ...
            <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} className="space-y-7">
                  ...
                  <div className="rounded-2xl border border-border bg-background p-6 md:p-8">
                    <div className="mb-5 flex items-center gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-xl"
                           style={{ background: `hsl(${storyColor} / 0.1)` }}>
              ...
              {activeStory && (
                <motion.div variants={fadeChild}>
                  <div className="flex items-start gap-4 rounded-2xl border p-5 md:p-6"
                       style={{
                         background: `hsl(${storyColor} / 0.03)`,
            ...
            <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }}
              ...
              <motion.div variants={fadeChild}>
                <Link to="/map" className="group block">
                  <div className="flex items-center gap-4 rounded-2xl border border-border bg-background p-5 transition-all hover:border-primary/20 hover:shadow-sm">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-primary/8 transition-colors group-hover:bg-primary/12">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-0.5 flex items-center gap-2">
                              ...
                              {r.short_description_ar && (
                                <span className="text-[0.64rem] text-muted-foreground/50 truncate">{r.short_description_ar}</span>
                              )}
                            </div>
                          </div>
  ...
  return (
    <div className="flex items-center gap-3.5 px-5 py-4">
      ...
      <div className="min-w-0">
        ...
        <p className={`text-[0.85rem] font-bold truncate ${
        ...
        <div className="space-y-2 p-4">
          {store.whatsapp && (
            <a href={`https://wa.me/${store.whatsapp}`} target="_blank" rel="noopener noreferrer" className="block">
              <div className="flex items-center gap-3 rounded-xl border border-emerald-500/15 bg-emerald-500/5 px-4 py-3 transition-all hover:border-emerald-500/25 hover:bg-emerald-500/8">
